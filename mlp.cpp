...
float * Net::mul(float ** w, int r, int c, float * vec){
    float * res;
    res = new float[r];
    for(int i = 0; i < r; i++){
        res[i] = 0;
...
float * Net::feed(float * input){
    for(int i = 0; i < lay_count-1; i++){
        ...
        fan_in[i+1] = mul(wt[i],node_count[i+1],node_count[i]-1,fan_f[i]);
...
//Update weights and adaptive learning rates
void Net::update_wt(bool adaptive_lrate){
    bool bad = false;
    if(adaptive_lrate){
        bad = ((total_error - prev_error) > L_THRESH);
        if(bad)
            if(lrate > MIN_LRATE)
                lrate *= MOD_LRATE_DEC;
        else
    ...
    for(int i = 0; i < lay_count-1; i++)
        for(int j = 0; j < node_count[i+1]; j++)
            for(int k = 0; k < node_count[i]; k++){
                if(!bad)
                    wt[i][j][k] += del_w[i][j][k];// + mtm*wt[i][j][k];
                del_w[i][j][k] = 0;
            }
    updated = true;
}
